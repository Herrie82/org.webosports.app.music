...
enyo.kind({
			...
			{kind: "Item", name: "Account", layoutKind: "HFlexLayout", align:"center", tapHighlight:true, className:"accounts-list-item enyo-text-ellipsis", components: [
				...
				{kind:"HFlexBox", style:"width:420px", align:"center", components:[
					{kind:"VFlexBox", align:"start", components:[
						{name: "accountName"},
						{name: "accountCategory", style:"font-size:11px; color:#8a8a8a; margin-top:1px;"}		// phase 3: category subtitle
					]},
	...
	onAccountsAvailable: function(inSender, inResponse) {
		...
		this._buildCapsMap(inResponse.templates);		// phase 3
	...
	listGetItem: function(inSender, inIndex) {
		...
		this.$.accountCategory.setContent(this._categoryLabel(a));		// phase 3
	...
	// --- phase 3: derive a friendly category label per account (from its template) ---
	_buildCapsMap: function(templates) {
		this._capsByTemplate = {};
		if (!enyo.isArray(templates)) { return; }
		for (var i = 0; i < templates.length; i++) {
			var t = templates[i];
			if (t && t.templateId) { this._capsByTemplate[t.templateId] = this._capsOf(t); }
		}
	},
	_capsOf: function(t) {
		var set = {};
	...
	_catPri: [["Music", $L("Music")], ["MAIL", $L("Email")], ["MESSAGING", $L("Messaging")], ["IM", $L("Messaging")],
		["DOCUMENTS", $L("Cloud & Photos")], ["PHOTO.UPLOAD", $L("Cloud & Photos")],
		["CALENDAR", $L("Contacts & Calendar")], ["CONTACTS", $L("Contacts & Calendar")], ["PHONE", $L("Phone")]],
	_categoryLabel: function(a) {
		var caps = (a && a.capabilityProviders) ? this._capsOf(a) : ((this._capsByTemplate && a && this._capsByTemplate[a.templateId]) || {});
		for (var i = 0; i < this._catPri.length; i++) { if (caps[this._catPri[i][0]]) { return this._catPri[i][1]; } }
		return "";
	},
